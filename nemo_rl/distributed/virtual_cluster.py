...
def init_ray(log_dir: Optional[str] = None) -> None:
    ...
    # vuvlm-local: cap num_cpus — host exposes 252 CPUs in container which
    # makes Ray's worker pool spawn fail. Override via NRL_RAY_NUM_CPUS env var.
    _ray_num_cpus = int(os.environ.get("NRL_RAY_NUM_CPUS", "64"))
    ...
    # vuvlm-local: Drop runtime_env and resources kwargs — passing either causes
    # raylet's dashboard_agent / runtime_env_agent subprocesses to die immediately
    # in this container (no agent log files get created). Workers will inherit
    # parent env via fork in local mode, which is sufficient for our smoke test.
    # include_dashboard=False — dashboard subprocess modules fail to start in this container.
    ray.init(
        log_to_driver=True,
        include_dashboard=False,
        _temp_dir=os.path.abspath(log_dir) if log_dir else None,
