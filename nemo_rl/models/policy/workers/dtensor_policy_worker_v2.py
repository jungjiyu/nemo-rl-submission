...
class DTensorPolicyWorkerV2(AbstractPolicyWorker, ColocatablePolicyInterface):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Reverted vuvlm-local force-FA2 — upstream default is faster for Mamba-heavy model
        # (attn is only 6 layers; FA2 explicit adds masking overhead without gain).
        attn_impl = (
            "flash_attention_2"
            if (self.enable_seq_packing and cp_size_cfg == 1)
            else ("sdpa" if cp_size_cfg > 1 else None)
        )
        # vuvlm-local: pre-AutoConfig probe — confirm what attn_impl we're about to request.
        try:
            import torch.distributed as _dist
            if (not _dist.is_initialized()) or _dist.get_rank() == 0:
                print(f"[vuvlm-local] PRE-CONFIG: requesting attn_impl={attn_impl!r} (cp_size_cfg={cp_size_cfg})")
        except Exception:
            pass
        ...
        model_config = AutoConfig.from_pretrained(
            ...
            attn_implementation=attn_impl,  # vuvlm-local: use the attn_impl chosen above (FA2 when CP==1)
            **hf_config_overrides,
        )
        # vuvlm-local: POST-AutoConfig probe — what model_config actually stored
        try:
            import torch.distributed as _dist
            if (not _dist.is_initialized()) or _dist.get_rank() == 0:
                print(f"[vuvlm-local] POST-CONFIG: model_config._attn_implementation={getattr(model_config, '_attn_implementation', '?')!r}")
        except Exception:
            pass
        ...
        with init_empty_weights():
            ...
            # vuvlm-local: verify FA2 actually picked up. Print requested + resolved impls.
            try:
                import torch.distributed as _dist
                if (not _dist.is_initialized()) or _dist.get_rank() == 0:
                    _resolved = getattr(self.model.config, "_attn_implementation", "?")
                    _attn_modules = {}
                    for _n, _m in self.model.named_modules():
                        _cn = type(_m).__name__
                        if "attention" in _cn.lower() or "attn" in _cn.lower():
                            _attn_modules[_cn] = _attn_modules.get(_cn, 0) + 1
                    print(f"[vuvlm-local] requested attn_impl={attn_impl!r} resolved={_resolved!r}")
                    print(f"[vuvlm-local] attention module class counts: {sorted(_attn_modules.items(), key=lambda x:-x[1])[:10]}")
            except Exception as _e:
                print(f"[vuvlm-local] attn impl probe failed: {_e}")
            if self.lora_enabled:
        ...
        # ----------------------------------------------------------------------
        # Enable activation checkpointing on the RADIO ViT block stack
        #   self.model.vision_model.radio_model.model.blocks
        # (timm VisionTransformer, 32 blocks of hidden=1280, nn.Sequential) so
        # block activations are recomputed in backward. Memory saved ≈ peak
        # ViT activations (~(UNIT-1)/UNIT of per-block activations).
        #
        # WHY NOT `policy.dtensor_cfg.activation_checkpointing` (the config flag):
        #   • world_size==1 path calls `model.gradient_checkpointing_enable()`
        #     on the top-level `NemotronH_Nano_VL_V2`, which does not implement
        #     it → AttributeError (same issue that forced patch #3 to keep the
        #     flag False in the smoke yaml).
        #   • Multi-GPU path picks `DefaultParallelizationStrategy` because
        #     NemotronH_Nano_VL_V2 is not in Automodel's PARALLELIZATION_STRATEGIES
        #     nor in VLM_MODEL_CLS_TO_LAYERS. Its fallback AC loop wraps
        #     `layer.mlp` / `layer.self_attn`, which do not match RADIO's
        #     `attn.qkv / attn.proj / mlp.fc1 / mlp.fc2` layout.
        #
        # HOW (clean approach — no state_dict key changes, LoRA-compatible):
        #   RADIO's own forward (see enable_cpe_support.py:_forward_cpe, bound
        #   to `model.forward_features`) already has a code path
        #     if getattr(self, 'grad_checkpointing', False):
        #         x = checkpoint_seq(self.blocks, x)   # hardcoded every=1
        #     else:
        #         x = self.blocks(x)
        #   We (a) monkey-patch the `checkpoint_seq` name in that module to a
        #   shim that injects `every=NRL_VISION_AC_UNIT`, then (b) flip
        #   `grad_checkpointing=True` on the timm VisionTransformer. No
        #   `checkpoint_wrapper` replacement of block modules → blocks stay a
        #   plain nn.Sequential, state_dict keys unchanged, LoRA targets
        #   (vision_model.radio_model.model.blocks.{i}.attn.{qkv,proj}, .mlp.{fc1,fc2})
        #   keep pointing at the same modules.
        #
        # SAFETY — Sequential grouping compatibility with RADIO block forward:
        #   timm Block.forward(self, x, attn_mask=None) — attn_mask defaults
        #   to None. _forward_cpe calls `checkpoint_seq(self.blocks, x)`, i.e.
        #   only x is threaded through the Sequential. So grouping multiple
        #   blocks per checkpoint unit via `every=N` is safe: nothing else is
        #   passed between blocks.
        #
        # ENV OVERRIDES (runtime, no code change; set via YAML
        # `policy.dtensor_cfg.env_vars:` — see smoke yaml for example):
        #   NRL_VISION_AC=0         → skip entirely (no checkpointing)
        #   NRL_VISION_AC=1         → enable (default)
        #   NRL_VISION_AC_UNIT=N    → checkpoint granularity (default 4)
        #     N=1  → 32 checkpoint boundaries, max memory save, max overhead
        #     N=4  → 8 boundaries, near-max save, low overhead  [recommended]
        #     N=8  → 4 boundaries, moderate save, very low overhead
        #
        # POSITIONING RATIONALE:
        #   • After `self.model_state_dict_keys` capture (line above): nothing
        #     about state_dict keys changes anyway (no structural wrap), but
        #     we stay consistent with patch #8's original positioning for
        #     uniformity.
        #   • After `apply_lora_to_linear_modules` (line ~346): LoRA already
        #     attached, so the checkpointed forward path also runs through
        #     LoRA adapters correctly.
        #   • Before FSDP parallelize: FSDP sees only plain blocks; the
        #     checkpointing is an inner runtime choice of _forward_cpe, not a
        #     structural layer — FSDP sharding is unaffected.
        #
        # HOW TO REVERT WITHOUT GIT:
        #   Delete everything from the line
        #     # vuvlm-local BEGIN  [patch #8 ...
        #   down to and INCLUDING the line
        #     # vuvlm-local END    [patch #8]
        #   (this entire block). No other file touched.
        #
        # SMOKE-TEST VERIFIER (look in stdout during worker init):
        #   [vuvlm-local patch#8] AC enabled on RADIO ViT: 32 blocks,
        #       unit=4 -> 8 checkpoint groups (every=4, grad_checkpointing=True)
        # ======================================================================
        import os as _nrl_os_p8
        _nrl_vision_ac_p8 = _nrl_os_p8.environ.get("NRL_VISION_AC", "1") != "0"
        try:
            _nrl_vision_ac_unit_p8 = max(1, int(_nrl_os_p8.environ.get("NRL_VISION_AC_UNIT", "4")))
        except ValueError:
            _nrl_vision_ac_unit_p8 = 4
        _nrl_vit_model_p8 = None
        try:
            _nrl_vit_model_p8 = self.model.vision_model.radio_model.model  # timm VisionTransformer
        except AttributeError:
            pass

        def _nrl_p8_log(_msg):
            try:
        ...
        if _nrl_vision_ac_p8 and _nrl_vit_model_p8 is not None:
            # Find the enable_cpe_support module (loaded via trust_remote_code, key
            # varies by model-repo hash) and patch its `checkpoint_seq` reference
            # to inject `every=UNIT`. We only patch once; subsequent re-inits
            # detect the sentinel attribute.
            import sys as _nrl_sys_p8
            _nrl_cpe_mod_p8 = None
            for _nm, _mod in list(_nrl_sys_p8.modules.items()):
                if _mod is None:
                    continue
                if _nm.endswith(".enable_cpe_support") and "RADIOv2" in _nm:
                    _nrl_cpe_mod_p8 = _mod
                    break
            if _nrl_cpe_mod_p8 is None:
                _nrl_p8_log(
                    "[vuvlm-local patch#8] WARN could not locate enable_cpe_support "
                    "module in sys.modules; falling back to default every=1 via "
                    "grad_checkpointing flag only"
                )
            else:
                if not getattr(_nrl_cpe_mod_p8, "_vuvlm_every_patched", False):
                    _nrl_orig_cs_p8 = _nrl_cpe_mod_p8.checkpoint_seq

                    def _nrl_patched_checkpoint_seq_p8(functions, x, every=None, *args, **kwargs):
                        # If caller (RADIO's _forward_cpe) doesn't specify every,
                        # honor our env-configured UNIT.
                        if every is None:
                            every = _nrl_vision_ac_unit_p8
                        return _nrl_orig_cs_p8(functions, x, every=every, *args, **kwargs)

                    _nrl_cpe_mod_p8.checkpoint_seq = _nrl_patched_checkpoint_seq_p8
                    _nrl_cpe_mod_p8._vuvlm_every_patched = True

            # Flip the timm VisionTransformer's grad_checkpointing flag so
            # _forward_cpe takes the checkpoint_seq branch.
            _nrl_vit_model_p8.grad_checkpointing = True
            _nrl_n_blocks_p8 = len(_nrl_vit_model_p8.blocks)
            _nrl_n_groups_p8 = (_nrl_n_blocks_p8 + _nrl_vision_ac_unit_p8 - 1) // _nrl_vision_ac_unit_p8
            _nrl_p8_log(
                f"[vuvlm-local patch#8] AC enabled on RADIO ViT: {_nrl_n_blocks_p8} blocks, "
                f"unit={_nrl_vision_ac_unit_p8} -> {_nrl_n_groups_p8} checkpoint groups "
                f"(every={_nrl_vision_ac_unit_p8}, grad_checkpointing=True)"
            )
        else:
            _nrl_reason_p8 = (
                "env NRL_VISION_AC=0"
                if not _nrl_vision_ac_p8
                else "no attribute model.vision_model.radio_model.model "
                     "(non-VL model or different VLM architecture)"
            )
            _nrl_p8_log(f"[vuvlm-local patch#8] SKIPPED vision encoder AC ({_nrl_reason_p8})")
        # vuvlm-local END    [patch #8]
        # ======================================================================
        ...
        # vuvlm-local: Allow partial DCP load — Nemotron VL has 1 derived buffer
        # (vision_model.radio_model.summary_idxs) not in HF safetensors. Verified
        # safe: no learned weights are missing. Default DefaultLoadPlanner has
        # allow_partial_load=False, which raises on the missing key. Patch the
        # default to True for this load.
        from torch.distributed.checkpoint.default_planner import DefaultLoadPlanner
        _orig_planner_init = DefaultLoadPlanner.__init__
        def _patched_planner_init(self, *args, **kwargs):
            kwargs.setdefault("allow_partial_load", True)
            _orig_planner_init(self, *args, **kwargs)
        DefaultLoadPlanner.__init__ = _patched_planner_init
