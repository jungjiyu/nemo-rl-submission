        ...
        for batch in dataloader:
            ...
            with timer.time("total_step_time"):
                ...
                with timer.time("reward_calculation"):
                    ...
                    # C.10 silent-masking / group-collapse fail-loud monitor.
                    # Catches (a) too many samples masked via loss_multiplier=0 from
                    # seqlen-truncation, (b) reward collapse where whole GRPO groups
                    # share a reward → advantage is 0. Env-gated so it can be
                    # disabled with NRL_DISABLE_C10_ASSERTS=1.
                    if not os.environ.get("NRL_DISABLE_C10_ASSERTS"):
                        try:
                            _lm = repeated_batch.get("loss_multiplier")
                            _lm_arr = (
                                _lm.detach().cpu().numpy()
                                if hasattr(_lm, "cpu")
                                else np.asarray(_lm)
                                if _lm is not None
                                else np.array([1.0])
                            )
                            _lm_mean = float(np.mean(_lm_arr))
                            _rw = (
                            ...
                            _np_groups = _rw.shape[0] // _ng if _ng > 0 else 1
                            _groups = (
                                _rw[: _np_groups * _ng].reshape(_np_groups, _ng)
                                if _np_groups > 0
                                else _rw.reshape(1, -1)
                            )
                            _pct_allzero_groups = (
                                float((_groups == 0).all(axis=1).mean())
                                if _groups.size
                                else 1.0
                            )
                            _group_stds = (
                            ...
                            _min_group_std = (
                                float(_group_stds.min()) if len(_group_stds) else 0.0
                            )
                            print(
                                f"[C.10 monitor] step={total_steps} "
                                f"loss_mult_mean={_lm_mean:.3f} "
                                f"n_rollouts={len(_rw)} "
                                f"reward_mean={_rw.mean():.3f} "
                            ...
                            assert _lm_mean > 0.5, (
                                f"C.10: step {total_steps}: loss_multiplier mean="
                                f"{_lm_mean:.3f} < 0.5 — too many samples silently "
                                f"masked. Check max_input_seq_length vs actual "
                                f"input_lengths."
                            )
                            assert _pct_allzero_groups < 0.95, (
                                f"C.10: step {total_steps}: "
                                f"{_pct_allzero_groups:.0%} of groups have all-zero "
                                f"reward — signal collapsed."
                            )
                        except AssertionError:
                            raise
                        except Exception as _e:
                            print(
                                f"[C.10 monitor] instrumentation failed (non-fatal): "
                                f"{_e!r}",
                                flush=True,
                            )
        ...
        while step < master_config["grpo"]["max_num_steps"]:
            ...
            with timer.time("total_step_time"):
                ...
                with timer.time("reward_calculation"):
                    ...
                    # C.10 monitor — async/DAPO path mirror of the sync-path
                    # instrumentation above. Same failure modes, different step var.
                    if not os.environ.get("NRL_DISABLE_C10_ASSERTS"):
                        try:
                            _lm = repeated_batch.get("loss_multiplier")
                            _lm_arr = (
                            ...
                            print(
                                f"[C.10 monitor/async] step={step} "
                                f"loss_mult_mean={_lm_mean:.3f} "
                                f"n_rollouts={len(_rw)} "
                                f"reward_mean={_rw.mean():.3f} "
                                f"reward_var={float(_rw.var()):.3f} "
                            ...
                            assert _lm_mean > 0.5, (
                                f"C.10/async: step {step}: loss_multiplier mean="
                                f"{_lm_mean:.3f} < 0.5 — too many samples silently "
                                f"masked. Check max_input_seq_length."
                            )
                            assert _pct_allzero_groups < 0.95, (
                                f"C.10/async: step {step}: "
                                f"{_pct_allzero_groups:.0%} all-zero groups — "
                                f"signal collapsed."
                            )
                        except AssertionError:
                            raise
                            ...
                            print(
                                f"[C.10 monitor/async] instrumentation failed "
                                f"(non-fatal): {_e!r}",
                                flush=True,
                            )
